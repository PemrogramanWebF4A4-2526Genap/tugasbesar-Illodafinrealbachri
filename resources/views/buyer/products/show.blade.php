...
<section style="padding:45px 70px;">
    ...
    <div style="display:grid; grid-template-columns:1.1fr 0.9fr; gap:55px; background:white; padding:35px; border:1px solid #ddd;">
        <div>
            @if($product->image)
                <img src="{{ asset('storage/'.$product->image) }}" style="width:100%; max-height:650px; object-fit:cover;">
            @else
                <div style="height:560px; display:flex; align-items:center; justify-content:center; background:#ddd;">No Image</div>
            @endif
        </div>
