<x-app-layout>
    <div style="padding:40px; max-width:1200px; margin:auto;">
        <h1 style="font-size:34px; font-weight:bold; margin-bottom:8px;">
            Dashboard Admin CAFFIN
        </h1>

        <p style="color:#666; margin-bottom:30px;">
            Ringkasan data marketplace fashion CAFFIN.
        </p>

        <div style="display:grid; grid-template-columns:repeat(5,1fr); gap:20px; margin-bottom:35px;">
            ...
            <div style="background:white; padding:25px; border-radius:14px; border:1px solid #ddd;">
                <p>Total Seller</p>
                <h2 style="font-size:32px; font-weight:bold;">{{ $totalSellers }}</h2>
            </div>

            <div style="background:white; padding:25px; border-radius:14px; border:1px solid #ddd;">
                <p>Total Buyer</p>
                <h2 style="font-size:32px; font-weight:bold;">{{ $totalBuyers }}</h2>
            </div>
            ...
            @forelse($latestOrders as $order)
                <div style="display:flex; justify-content:space-between; padding:15px 0; border-bottom:1px solid #eee;">
                    <div>
                        ...
                        <p style="color:#666;">Buyer: {{ $order->buyer->name }}</p>
                    </div>
